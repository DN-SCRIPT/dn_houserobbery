...
RegisterServerEvent('dn:checkitem')
AddEventHandler('dn:checkitem', function()
    ...
    if Config.RequiredItem then
        if xPlayer.getInventoryItem(Config.ItemRequired).count >= 1 then -- or if xPlayer.getInventoryItem('Config.ItemRequired', 1) then  ####  for users who do not use ox inventory
            xPlayer.triggerEvent('dn:startminigame')
        else
...
RegisterServerEvent('dn:removeitem')
AddEventHandler('dn:removeitem', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    xPlayer.removeInventoryItem(Config.ItemRequired, 1)
end)


RegisterServerEvent('dn:giveloot')
AddEventHandler('dn:giveloot', function ()
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('dn:contpolice', function(source, cb)
    local police = 0
...
ESX.RegisterServerCallback('dn:house:cooldown', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('dn:time')
AddEventHandler('dn:time', function()
    Config.Lastrob = os.time()
end)

RegisterServerEvent('dn:sendcall')
AddEventHandler('dn:sendcall', function(coords)
    TriggerClientEvent('dn:policecall', -1, coords)
end)
