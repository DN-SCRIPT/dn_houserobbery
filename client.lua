...
RegisterNetEvent('dn:checkpolice')
AddEventHandler('dn:checkpolice', function()
    Citizen.CreateThread(function()
        while true do
            s = 1000
            local canrob = nil
                ESX.TriggerServerCallback('dn:house:cooldown', function(can)	
                    if can then
                        ESX.TriggerServerCallback('dn:contpolice', function(cb)
                            canrob = cb
                        end)
                        while canrob == nil do
                            Wait(10)
                        ...
                        if canrob then
                            TriggerServerEvent('dn:time')
                            TriggerEvent('dn:startrobbery')
                            exit = false
                        else
...
RegisterNetEvent('dn:startrobbery')
AddEventHandler('dn:startrobbery', function()
                        ...
                        s = 5
                            if Config.OxTarget then
                                ...
                                if IsControlJustReleased(0,38) and PlayerData.job.name ~= Config.PoliceJob then
                                    if start == true then
                                    TriggerServerEvent('dn:checkitem')
                                    else
...
RegisterNetEvent('dn:startminigame')
AddEventHandler('dn:startminigame', function()
    ...
    local success = lib.skillCheck({ 'easy', 'easy', 'easy', { areaSize = 60, speedMultiplier = 1 }, 'easy' })
        ...
        if success then
            ...
            TriggerServerEvent('dn:removeitem')
            ClearPedTasks(PlayerPedId())
        end
end)

RegisterNetEvent('dn:leavehouse')
AddEventHandler('dn:leavehouse', function()
    start = false
    SetEntityCoords(PlayerPedId(),alllocations[randomlocation].door,0,0,0,0)
end)
                        ...
                        if GetDistanceBetweenCoords(coords,alllocations[randomlocation].loot[i], true) < 1 then
                                if IsControlJustReleased(1, 51) and not collected then
                                        ...
                                        if cont == 1 then   --this will be used so that when the thief has checked at least 1 places, an announcement will be sent to the police, so that the thief will have a chance to escape.
                                            ...
                                            TriggerServerEvent('dn:sendcall', alllocations[randomlocation].door)
                                        elseif cont == 4 then
                                            revising = false
                                            can_review = false
                                            all_reviewed = true
...
RegisterNetEvent('dn:policecall')
AddEventHandler('dn:policecall', function(coords)
    ...
    if inside then
        ...
        if revising == false then
            if all_reviewed == false then
                ...
                TriggerServerEvent('dn_logsloot')
            else
                all_reviewed = false
            end
        end
...
function animation()
    ...
    TriggerServerEvent('dn:giveloot')
end
